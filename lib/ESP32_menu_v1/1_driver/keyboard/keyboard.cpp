...
/*
    函数名字：keyboard_interrupt
    函数功能：作为键盘扫描的中断函数使用,扫描键盘，计算其他参数
    返回值：没有
    参数：没有
*///
void keyboard_interrupt(){
    ...
    if( keyboard_status.press_time != 0 )
    keyboard_status.last_press_time = keyboard_status.press_time;

    if( keyboard_status.key_value != 0 )
    keyboard_status.last_key_value = keyboard_status.key_value;
}
...
void init_keyboard_timer(){
    ...
    switch(USE_TIMER){
        ...
        default:{//选择硬件定时器
            ...
            timerAttachInterrupt( timer , keyboard_interrupt, false );//用于将中断处理函数与特定的定时器关联起来
...
/*
    函数名字：get_last_key
    函数功能：返回松开按键那一瞬间的值，没按返回KEY_NULL
    返回值：
        类型：uint8_t
        意义：按键的值
    参数：没有
...
/*
    函数名字：get_last_press_time
    函数功能：返回松开按键那一瞬间的按下时间，没按返回KEY_NULL
    返回值：
        类型：uint16_t
        意义：按下时间
    参数：没有
*///
uint16_t get_last_press_time(){

    __DEBUG_1("get_last_press_time() = ")
    __DEBUG_1(keyboard_status.last_press_time)

    return keyboard_status.last_press_time;
}